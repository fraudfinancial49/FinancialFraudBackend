...
def _upsert_profile(db: Session, row: Dict[str, object], label: str, threat_score: float) -> None:
    ...
    # Threat score is owned by the realtime honeypot_service escalation path (the one
    # actually wired to auto-block) -- this batch job must not clobber it on every run.
    # Only seed a baseline score for a profile this job is discovering for the first time.
    if is_new:
