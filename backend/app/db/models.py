...
class SafeVaultTransaction(Base):
    ...
    # NOTE: "otp_verified" is the single terminal state for successful step-up verification,
    # reached identically whether the customer verifies via /transactions/{id}/verify-otp or
    # an admin verifies on their behalf via /vault/otp -- both delegate to otp_service.
...
class HoneypotSession(Base):
    ...
    # --- Actual Network Tracking ---
    actual_ip = Column(String(64), nullable=True, index=True)
...
class HoneypotEvent(Base):
    ...
    # --- Telemetry & Interaction Fields ---
    stage = Column(String(40), nullable=True, index=True)
...
class Customer(Base):
    ...
    account_id = Column(String(64), nullable=False, unique=True, index=True)  # == Transaction.name_orig/name_dest
