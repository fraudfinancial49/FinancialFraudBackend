...
def analytics_summary(
    ...
    row = (
        db.query(
            ...
            func.coalesce(func.sum(case((models.ModelPrediction.routing_decision == "otp_verification", 1), else_=0)), 0),
            func.coalesce(func.sum(case((models.ModelPrediction.routing_decision == "auto_reject", 1), else_=0)), 0),
    ...
    total, volume, approve_ct, vault_ct, auto_reject_ct, honeypot_ct, avg_risk, avg_latency = row

    # Blocklist short-circuits never reach the scoring pipeline, so they have no
    # ModelPrediction row -- they're only ever recorded as an AutoRejectedTransaction
    # with reason="blocked_account". Counted separately, on the same date range.
    block_ct = (
        db.query(func.count(models.AutoRejectedTransaction.id))
        .join(models.Transaction, models.Transaction.id == models.AutoRejectedTransaction.transaction_id)
        .filter(
            models.AutoRejectedTransaction.reason == "blocked_account",
            models.Transaction.timestamp >= start_dt, models.Transaction.timestamp < end_dt,
        )
        .scalar() or 0
    )

    total_all = total + block_ct
    flagged_ct = vault_ct + auto_reject_ct + honeypot_ct + block_ct
    fraud_rate = (flagged_ct / total_all) if total_all > 0 else 0.0
    ...
    return schemas.TransactionAnalyticsSummary(
        ...
        auto_reject_count=auto_reject_ct,
...
def analytics_timeseries(
    ...
    block_day_bucket = func.date(models.Transaction.timestamp)
    block_rows = (
        db.query(block_day_bucket.label("day"), func.count(models.AutoRejectedTransaction.id))
        .join(models.Transaction, models.Transaction.id == models.AutoRejectedTransaction.transaction_id)
        .filter(
            models.AutoRejectedTransaction.reason == "blocked_account",
            models.Transaction.timestamp >= start_dt, models.Transaction.timestamp < end_dt,
        )
        .group_by(block_day_bucket)
        .all()
    )
    block_ct_by_day = {str(day): ct for day, ct in block_rows}

    return [
        schemas.TransactionTimeseriesPoint(
            date=str(day),
            total=total + block_ct_by_day.get(str(day), 0),
            approve_count=approve_ct,
            vault_count=vault_ct,
            auto_reject_count=auto_reject_ct,
            block_count=block_ct_by_day.get(str(day), 0),
            honeypot_count=honeypot_ct,
            flagged_count=vault_ct + auto_reject_ct + honeypot_ct + block_ct_by_day.get(str(day), 0),
        )
        for day, total, approve_ct, vault_ct, auto_reject_ct, honeypot_ct in rows
    ]
...
def list_transactions(
    start_date: Optional[date] = Query(None),
    end_date: Optional[date] = Query(None),
    routing_decision: Optional[str] = Query(None, pattern="^(approve|otp_verification|auto_reject|honeypot)$"),
